...
class Solution:
    def stonks(self, prices):
        ...
        max_dist = prices[1] - prices[0]
        index_one = prices[0]
        index_two = prices[1]

        for x in range(len(prices)):
            for y in range(x, len(prices)):
                if prices[y] - prices[x] > 1 and prices[y] - prices[x] > max_dist:
                    max_dist = prices[y] - prices[x]
                    index_one = x
                    index_two = y
        
        if max_dist > 0:
            prices.pop(index_one)
            prices.pop(index_two)
            max_dist_two = max_dist = prices[1] - prices[0]
            for x in range(len(prices)):
                for y in range(x, len(prices)):
                    if prices[y] - prices[x] > 1 and prices[y] - prices[x] > max_dist_two:
                        max_dist_two = prices[y] - prices[x]
            if max_dist_two > 1:
                return max_dist + max_dist_two
        return 0
